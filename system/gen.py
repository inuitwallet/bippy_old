...
def genBIPKey(currency, passphrase, entropy='',privateKey=''):
	"""
	...
	publicAddress = address.publicKey2Address(address.privateKey2PublicKey(privateKey), int(cur['version']), prefix, int(cur['length']))
	#BIP38 encryption
	BIP = bip38.encrypt(privK256, publicAddress, str(passphrase), 8)
	return BIP, publicAddress
	
def encBIPKey(privK, cur, passphrase):
	"""
	Encrypt an existing private key with BIP38
	"""
	#we need to check what type of private key we are working with and change it to raw
	#this function doesn't currently do anything
	privK = enc.privKeyVersion(privK, cur)
	#once we have this we can use the function above to generate the BIP keys
	BIP, publicAddress = genBIPKey(cur, passphrase, None, privK)
	return BIP, publicAddress
